...
*Overview of segmentation paradigms.* **(Upper left)** *Classic supervised ML trains a fixed model per task and fails on unseen tasks.* **(Lower left)** *Standard prompt-conditioned models use prompts to define new tasks but offer no correction mechanism.* **(Right)** *LISP-Net combines interactive prompt conditioning with a single dense 2D prompt. Within-volume visual correspondence propagates the user's annotation with strong alignment to the user's intent and high accuracy over medium-range offsets. Structural drift is handled automatically by SSF or manually by the user, both of which refresh the prompt context.*
<br>
...
│   ├── ssf.py                  # Self-monitoring Slice Feedback strategies
...
For 3D volumes, use `inference/inference_volume.py` which orchestrates slice-by-slice prediction with optional SSF (Self-monitoring Slice Feedback) and IFL (Oracle Interactive Feedback Loop).

SSF in `inference/ssf.py` uses a `ConfidenceDropStrategy` with buffer aggregation; additional legacy strategies (`RelativeSSIMStrategy`, `MaskDiceStrategy`) are also available.
