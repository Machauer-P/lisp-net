"""
inference/ssf_buffer_mean.py
============================
Drop-in replacement for ``inference/ssf.py`` that uses **buffer-mean**
(majority vote at threshold 0.5) instead of buffer-minimum for prompt
refresh aggregation.  All class names, strategies, and the public API are
identical — only the aggregation function differs.

Usage: replace the import in the 3D benchmark script or VolumeInference::

    # from inference.ssf import SSFController      # buffer-minimum
    from inference.ssf_buffer_mean import SSFController  # buffer-mean
...
predictions (buffer-mean strategy, thresholded at 0.5).
...
Usage
-----
    from inference.ssf_buffer_mean import SSFController, RelativeSSIMStrategy
    ...
    if fired:
        # new_mask is (1, 128, 128, 1) binary TF tensor — the buffer-mean refresh
        ...
...
class ConfidenceDropStrategy(BaseSSFStrategy):
    ...
    and SSF does **not** fire.  Empty masks carry no segmentation information
    and would dilute the buffer-mean.

    Parameters
    ----------
    drop_fraction : float
        Relative confidence drop that triggers SSF.  Default 0.05.
    """
    ...
    def check(
        ...
        if not fg_mask.any():
            # |F| = 0: skip — empty mask carries no information and would
            # dilute the buffer-mean.
            if self.debug:
                print(
...
class SSFController:
    ...
    Uses **buffer-mean** (majority vote at threshold 0.5) for prompt refresh
    aggregation, as opposed to the buffer-minimum in ``inference/ssf.py``.
    ...
    buffer_size : int
        Rolling buffer depth for the buffer-mean prompt refresh.  Default 4.
    ...
    def update(
        ...
        strategy whether SSF should fire.  If yes, computes the buffer-mean
        refresh mask (majority vote at threshold 0.5), clears the buffer
        (seeding it with the refresh mask), and resets the strategy.
        ...
        (fired, new_mask_128)
            ...
            new_mask_128 : (1, 128, 128, 1) buffer-mean refresh mask TF tensor,
        ...
        # dilution of future buffer-mean refreshes.
        if pred_b_np.sum() > 0:
            self._buffer.append(pred_binary_128)
    ...
    def _buffer_mean(self):
        """
        Return buffer-mean mask as (1, 128, 128, 1) binary TF tensor.

        Computes the per-pixel mean across buffered binary masks and
        thresholds at 0.5 (majority vote: a pixel must appear in at least
        half the buffer entries to be included in the refresh mask).
        """
        stacked  = tf.stack(list(self._buffer), axis=0)   # (N, 1, 128, 128, 1)
        mean_val = tf.reduce_mean(stacked, axis=0)         # (1, 128, 128, 1)
        return tf.cast(mean_val >= 0.5, tf.float32)
